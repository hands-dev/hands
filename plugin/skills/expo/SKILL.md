...
Run this whole loop each time you're invoked (`/loop /yc:expo` — pacing in "Heartbeat, pacing &
...
- **One bundled read per pass:** `yc_board({ full: true })` returns everything — `peers` (with
...
- **FYIs are non-waking:** `yc_send({ ..., wake: false })` lands on the next natural drain. Only
...
Your inbox is the `expo.notify` file (the `notify` path from `yc_paths`) — every waking message,
...
- **Auto-resolve:** `yc_answer({ id, answer, by: "expo", priority })` — note it in one line so the
  principal can see (and undo) it.
- **Escalate:** `yc_escalate({ id, recommendation, priority })`, fire the desktop ping (below),
...
`stations.overrides` in `yes-chef.config.json`. Tag every ticket with a recommended tier —
...
**Scale the line (if enabled):** when config `stations.allowScaling` exposes `yc_station_add` /
`yc_scale` / `yc_station_remove`: under-staffed specials with nobody idle → open stations (relay
...
normal `yc_delegate` titled `Hands: <dish>` — stations do the prep, never the merge/deploy itself.

## 6. The board (external): GitHub awareness

If config `gh.poll` is on: once every few passes (network call), run `yc_gh_poll`. It records what
...
The concrete things only the principal can do. Add (`yc_todo_add`) the moment you see one — an
...
(`yc_todo_update({ state: "done", doneSignal })`) only on a strong signal (PR merged, commit,
...
line narrative with `yc_digest_note`: what moved, what's blocked, what tomorrow opens with. It
...
- The **Monitor is the wake signal** — bus traffic reaches you in sub-seconds. The `/loop` timer
  is the *time-based* beat only: self-pace `ScheduleWakeup` at **~900s** (prompt `/loop /yc:expo`)
...
- **Compaction cadence.** You accrete context like any long-lived pane; compact on a **quiet
  ...
  **`/compact`** instead of `/loop /yc:expo` — the persistent Monitor stays armed across the
...
`yc_answer`, escalate via `yc_escalate` — if you catch yourself deciding something significant in
