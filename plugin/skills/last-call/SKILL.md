...
For anything still `in_progress` or blocked (including a dish that didn't clear step 2's merge
sweep), leave it in a state a station can pick up on wake without you:
`hands_task_update` with a clear `result`/status note. If the principal named specific upcoming
work, file it now with `hands_delegate` so it's waiting rather than something tomorrow has to ask
for.
