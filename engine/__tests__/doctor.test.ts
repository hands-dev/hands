...
describe("inbox monitor check (hands#90) — doctor never looked at this at all before", () => {
  ...
  // The real scanner (spawn a real `tail`, scan the real process table) is proven ONCE, in
  // watchers.test.ts's dedicated suite (hands#105) — that's a real-process integration test,
  // deliberately allowed to be slower and Linux-only. This describe block's own subject is
  // doctor's severity-mapping given a liveness RESULT, which doesn't need a real process to
  // exercise — injecting the result keeps these tests deterministic-by-construction rather than
  // deterministic-by-luck (a real spawn+scan here previously flaked under full-suite process
  // churn, since the scan can transiently miss an already-alive process under load).
  it("passes when the monitor is reported alive", () => {
    oneStation();
    const report = runDoctor({ cwd: repo, env, checkMonitorAlive: () => true });
    expect(check("station-1.monitor", report)?.severity).toBe("ok");
  });

  it("warns, not fails, when the platform can't be inspected at all", () => {
    oneStation();
    const report = runDoctor({ cwd: repo, env, checkMonitorAlive: () => null });
    const monitor = check("station-1.monitor", report);
    expect(monitor?.severity).toBe("warn");
    expect(monitor?.detail).toContain("UNVERIFIED");
  });
});
