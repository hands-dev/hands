 ...
 *   1. Overall utilization — stations active/idle/offline + how the fleet is
 *      resourced across the ranked daily priorities.
 *   2. Stations — a 2-column grid: canonical id (station-<n>), current task, and
 *      which daily priority it serves (assigned = filled badge, inferred from
 *      branch = outlined ~badge).
 *   3. Expo effectiveness — the expo's OWN hindsight verdict on its
 *      recommendations (validated vs later-contradicted), recency-weighted. Not
 *      the principal's acceptance rate.
 *
 * Agents are addressed by canonical id everywhere (expo, station-<n>) — there
 * is no persona/roster layer. Polls /api/state every 2s.
 */

/** Render the dashboard page (principal name comes from yes-chef.config.json). */
  ...
  function renderExpo(s){
    ...
    document.getElementById('fore-body').innerHTML =
      ...
      '<p class="card-desc" style="padding:0 0 12px">The expo grades its own calls in hindsight — did each recommendation hold up, or did a later finding overturn it? '+
        'Score = held up ÷ (held up + overturned), recency-weighted. It measures the expo\\'s judgment, not whether '+esc(PRINCIPAL)+' accepted its advice.'+
