...
1. Call \`hands_brief({ craft: "${entry.slug}", task: "<one line — what you were asked to do>" })\`
   first. This registers the dispatch and returns a \`briefId\` — note it, you need it below.
2. Invoke \`Skill({ skill: "craft-${entry.slug}" })\` — your operating manual. It tells you how to
   pull your current book/mise (via \`hands_mise\`, using the briefId from step 1) before you start.
...
**First**, before anything else: call \`hands_mise({ briefId: <the id hands_brief gave you> })\` —
pulls your current book/mise, the sibling craft roster, and a read-in command if you're stale.
Trust what it tells you before re-deriving anything yourself.
...
mise: <path/command — only if it differs from what hands_mise told you>
 ...
 * hands_brief/hands_mise round-trip needed by the DISPATCHER (the generated agent does that
 * itself, per its own system prompt above).
 *
 ...
 * hands_brief/hands_mise (live MCP calls) until that station restarts.
 *
 ...
 * which need the hands_brief/hands_mise fallback (founded/edited since the
 * last `hands craft sync` or provisioning).
 */
export function formatRosterContext(entries: CraftRosterEntry[], targetDir: string): string {
  ...
  return (
    ...
    'and equips itself. "not yet synced" → fall back to hands_brief({ craft: "<slug>" }), paste the ' +
    "returned chit into a general-purpose Agent's prompt instead. hands_crafts gives the full roster " +
    "on demand."
  );
}
